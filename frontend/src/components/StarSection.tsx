...
export const StarSection: React.FC<StarSectionProps> = ({
  ...
  return (
    <Stack direction={"row"} align={"center"} spacing={2} ml="auto">
      {!isOnMainPage && data?.countUsers ? (
        <Text mr={2} fontSize={"small"}>
          {Math.round((organization.points / data!.countUsers) * 100)}% of
          people have starred this organization{" "}
        </Text>
      ) : null}
      ...
      <IconButton
        ml="auto"
        icon={<StarIcon />}
        colorScheme={organization.voteStatus === 1 ? "yellow" : undefined}
        m={2}
        onClick={() => {
          if (meData?.me) {
            vote({
              ...
              update: async (cache) => {
                await updateAfterVote(
                  organization.voteStatus === 1 ? -1 : 1,
                  organization.id,
                  cache
                );
              },
